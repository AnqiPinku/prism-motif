...
| 卡农进(循环下行 bass)| I-V/vi-vi-iii-IV-I/iii-ii-V | C-E7-Am-Em-F-C/E-Dm-G | 卡农、婚礼、日系 | bass 级进下行(要严格顺阶,把 E7 换 G/B) |
...
调 `add_midi_notes`:每和弦占 1 整小节(4/4 = 4 拍):start_beats=0/4/8/12(单位是拍,不是小节),length_beats=4,velocity=80。

**Bass 单独一轨走根音**,音区 F1–F2(MIDI 29–41)一带(听得见 bass 又不闷)。MIDI:

- Dm 小节 → D2 = `38`
- Bb 小节 → Bb1 = `34`
- F 小节  → F1  = `29`(太低就用 F2=41)
- C 小节  → C2  = `36`
...
1. **属七推进** — 在 F(I)前一拍插它的属七 V7 = C7,变成 `Dm | Bb-C7 | F | C`。C7 MIDI `[60, 64, 67, 70]` = C4 E4 G4 Bb4。Bb→A 半音进 F 的 A,推动感立刻出。
2. **借用 iv 换 IV** — 把 Bb 换成 Bbm(F 调的 iv 借用)。Bbm MIDI `[58, 61, 65]` = Bb3 Db4 F4。Db 是特征音,忧郁度直接上一档。用在 verse 最后一句效果最好。
3. **末尾 tritone sub** — 把最后的 C(V)换成 F#7(bII7)。bass 走 F#→F,neo-soul 感立刻出。F#7 MIDI `[54, 58, 61, 64]` = F#3 Bb3(=A#3) C#4 E4;bass F#1 = `30`。

## Voicing 具体指令

- **左手 bass 单音在 F1–F2(MIDI 29–41)**,右手三/四音和弦放在 C4–C5(MIDI 60–72)之间不打架。
