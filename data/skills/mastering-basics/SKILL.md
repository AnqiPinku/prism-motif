...
**先明确:平台数字是"播放归一化参考线",不是"母带必须交这个数"。** Spotify/Apple/YouTube 播放端把音量对齐到 ≈ **-14 LUFS**:交 -8 会被调低(你在限制器里花掉的动态不退还),交 -16 有的平台拉高、有的不动。所以响度目标由**风格审美 + 动态预算**决定;参考线只说明一件事——推得再响,归一化后也不会更响。

| 场景(按风格选,不是按平台) | 常见交付区间 (Integrated) | True Peak | LRA 参考 |
|---|---|---|---|
...
- **给不同平台各做一版母带**:主流流媒体播放端都归一化到约 -14 LUFS,一版落在你风格区间的母带即可通吃;只有 club/无归一化场景值得考虑单独一版更密的。别过度加工。
