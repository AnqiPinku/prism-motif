...
description: 当你混完一首歌准备上传 Spotify/Apple Music/YouTube/SoundCloud 前的最后一步——按风格和目标平台选择响度策略、保证真峰值不炸、保住动态范围。触发词:mastering、母带、响度、LUFS、真峰值、-14 LUFS、-9 LUFS、限制器、maximizer、平台响度、streaming loudness、normalization、发布前处理、上传前处理、我的歌上传后比别人小声、Spotify 播放像盖了层棉被、preview 太小声。也用于诊断响度问题、判断"这混音能不能上母带",给出各平台响度参考值、按风格的目标策略和母带链的具体插件顺序与参数。不用于混音阶段的 EQ/压缩(那是 mixing),也不用于商业级 stem mastering 交付(那需要专门工程师)。
---
...
当你混完一首歌,拿去 Spotify/Apple Music/YouTube/SoundCloud 发布前的最后一步——按风格和目标平台选响度策略、保证真峰值不炸、保住动态范围(LRA)。触发词:mastering、母带、响度、LUFS、真峰值、-14 LUFS、-9 LUFS、限制器、maximizer、平台响度、normalization、streaming loudness、送母带、preview 时太小声、发布前处理。也用于:诊断"为什么我的歌上传后比别人小声"、"为什么 Spotify 播放我的歌像盖了层棉被"。不用于:混音阶段的 EQ/压缩(那是 mixing),也不用于 stem mastering(把混音分组交给专门母带工程师的商业流程)。
...
**先明确:平台数字是"播放归一化参考线",不是"母带必须交这个数"。** 各平台不能合并成一个 -14 LUFS 规则:

- **Spotify** 官方 Normal 播放级别以 **-14 LUFS** 为参考,但用户可选 Loud/Normal/Quiet,部分设备也不执行归一化；
- **Apple Music** 的 Sound Check 会对齐歌曲间响度,官方公开说明不提供可当成交付目标的固定 LUFS 数；
- **YouTube / YouTube Music** 的播放处理与 Spotify 不是同一份规范,不要把 -14 LUFS 写成其官方交付标准。

因此响度目标由**风格审美 + 动态预算**决定;平台参考只用于预判播放端可能怎样调节。官方行为会变化,发布前按目标平台最新文档复核。来源:[Spotify Loudness Normalization](https://support.spotify.com/ee-en/artists/article/loudness-normalization/)、[Apple Sound Check](https://support.apple.com/109331)、[YouTube Stable Volume](https://support.google.com/youtube/answer/14106294)。

| 场景(按风格选,不是按平台;以下均为 heuristic 起点) | 常见交付区间 (Integrated) | True Peak | LRA 参考 |
|---|---|---|---|
| 流行/摇滚/民谣,主发流媒体 | **-14 到 -10 LUFS** | ≤ **-1.0 dBTP** | 6–9 LU |
| EDM / hip-hop,密度审美 | -9 到 -6 LUFS(有意用动态换密度) | ≤ -1.0 dBTP | 3–6 LU |
| 古典 / jazz / 电影配乐 | -18 到 -16 LUFS | ≤ -1.0 dBTP | 10–20 LU |
| club 现场 / Beatport / 无归一化场景 | -8 到 -6 LUFS | ≤ -1.0 dBTP | 3–5 LU |

选定区间后,它就是这版母带的"合同",下面所有闭环都对着它验收。

术语先扫一遍(第一次遇到就记住):
- **LUFS** — Loudness Units Full Scale,人耳感知响度。**同一单位有三个时间尺度,不可混用**:
  - **Integrated(整体)**:全曲带门限一个数,平台归一化读的就是它;
  - **Short-term(短期)**:3 秒滑窗。`measure_loudness` 的 `short_term_series` 给逐秒曲线,回答"副歌哪几秒过响";
  - **Momentary(瞬时)**:400ms,`momentary_max_lufs` 抓单发瞬态。
  注意:对渲染出来的**片段**跑 measure_loudness,得到的是"该片段的 integrated",不能称之为 short-term(两者门限规则不同)。
...
- **Normalization** — Spotify Normal 以 -14 LUFS 为播放参考；更响的母带通常会被调低,但实际行为还受用户播放级别、headroom 和设备支持影响。
...
6. listen_subjective(path) → 听 muddy/harsh/pumping;若报告 "pumping at 1:23",先用
   measure_loudness 的 short_term_series 在 83s 附近核对——曲线剧烈起伏(相邻窗 ±2 LU 以上)
   坐实限制器泵浦,回步骤 3 减 1dB;曲线平稳则多半是编曲本身的强弱设计,别错怪限制器
7. add_marker 在 REAPER 工程标实际测量值,例如 `MASTER v1 -11.2LUFS -1.0dBTP`,便于回溯
```
...
- [ ] Integrated LUFS 落在**你选定的策略区间**内(见上表;区间是风格决定,不是平台规定)
- [ ] True peak **≤ -1.0 dBTP**(`measure_loudness` 确认)
- [ ] LRA 在选定的风格策略区间(见上表;不是平台硬性要求)
- [ ] 曲首曲尾各留 **100–500ms** 静音,防止播放器切歌爆音
- [ ] 手机外放 + 耳机 + 车载三处 A/B 同风格商业发行版
- [ ] `add_marker` 在工程标实际 integrated LUFS 与 true peak,不要把 -14 写死

## Anti-patterns

- **追响度战争**:把流行歌推到 -6 LUFS 交 Spotify Normal,播放端会大幅调低,动态被你毁了却没多响一分。把 -14 LUFS 当 Spotify Normal 的试听参考,不是所有平台或所有风格的统一母带目标。
...
- **把所有平台说成同一个 -14 LUFS 规范**:Spotify、Apple Music、YouTube 的公开说明和播放选项不同。通常先交一版符合风格与动态预算的高质量母带,再用各目标平台的当前播放行为做预览;只有 club、广播或其他明确交付规范要求时才另做版本。
