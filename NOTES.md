...
13. Gold 分三层：Gold A 是开发期离线回归底座，当前已完成并进入维护态；Gold B 是产品主路径稳定后的单次 G001 真机生命周期冒烟；Gold B 通过后进入 Gold C Prep，声明—工程一致性评分、G010 确定性故障注入、至少 30 个唯一任务的 catalog 和离线评测契约必须在候选冻结前完成。随后 Gold C 才对冻结的发布候选、模型、Provider、Prompt、Skill、工具和任务版本执行 10-task canary；无变更时 canary 计入最终全集并继续剩余固定任务，有变更时全部结果失效，重新冻结并从零开始。完整 Gold C 只阻塞发布，不阻塞产品主路径补全和 Phase 4 结构收口。
...
1. 尚未从普通用户视角完成产品完整性盘点；安装/连接、三模式任务、执行反馈、结果核验、失败恢复、线程保存和退出是否形成稳定闭环，需要逐项以真实界面和行为核对。
2. G001 出现过实际 MIDI 和弦顺序与 Agent 最终文字不一致；`composition-workflow` 已要求写后读回，这既是产品可靠性问题，也是 Gold C 前必须补齐的评分证据。
3. G001 曾出现一次桥接超时，虽由 Agent 成功恢复，产品主路径仍需要可理解的连接健康、超时、重试和失败恢复反馈。
4. Phase 3.4 的真实制作人复核与响度匹配 A/B/测量证据仍未完成；可做针对性验证，不等待完整 Gold campaign。
5. App.tsx 与 Gateway Handler 的结构债务进入 Phase 4，但只能在现有离线回归与 Soak 保护下渐进收口。Gold B 真机冒烟和 Gold C 的 10/30-task 门属于已知延期的发布前工作，不是当前施工入口。
