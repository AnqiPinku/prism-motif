...
控制：代码级风险元数据；信任模式只放行明确分类的 REAPER 工程内编辑，批处理逐项检查；录音、外部调用、任意代码/命令和文件覆盖/删除不可绕过。
...
|---|---|
| read | 自动执行 |
| write | 请求确认；当前会话信任模式可放行已知的 REAPER 工程内编辑 |
| destructive | 请求确认；仅 Policy 显式标记的可撤销工程内删除/替换可由信任模式放行 |
| execute | 始终确认并展示代码/命令 |
| external | 按目标与副作用确认 |
| record | 始终确认 |

未知工具不得默认为 read，也不得由信任模式放行。`batch` 不沿用一个固定风险：Gateway 必须展开并检查所有子调用；只要包含未知 ReaScript、Lua、录音、外部调用，或显式路径的文件覆盖/删除，整批继续要求确认。应用管理的临时分析渲染仍按已知 `write` 处理。
