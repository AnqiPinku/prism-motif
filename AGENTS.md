...
- **验收**：能连上 `A:/Prismcode/reaper-mcp/server/reaper_mcp_server.py`，`list_tools()` 拿到 ~20 个工具，`call_tool("reaper_status",{})` 有返回。
...
```json
{ "servers": [
  {"name":"reaper","enabled":true,"command":"python","args":["A:/Prismcode/reaper-mcp/server/reaper_mcp_server.py"]}
] }
```
