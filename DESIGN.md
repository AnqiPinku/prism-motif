...
10. 信任模式始终开启且无独立 UI：已知 REAPER 工程内的编辑、以及显式标记为可撤销的
    删除与替换自动执行；录音、外部调用、任意代码或命令、显式的文件覆盖与删除仍然
    需要确认，这条底线不依赖任何开关。新启用 MCP 的陌生工具不在信任白名单内，
    Policy 仍逐个要求确认。`batch` 必须展开到子调用逐个判定，不能整体放行。
    长期方向是以工程操作记录与回滚作为主要安全补偿。
