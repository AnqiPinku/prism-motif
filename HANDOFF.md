...
- Phase 2 已完成：当前 Python 93 项（含 Gold Harness 11 项）、前端 11 项、Rust 3 项、真实 MCP 集成、工具契约、仓库卫生和 20+ 回合 Soak 门禁均通过；
...
- Phase 3.5 已建立 6 个固定工程、首批 10 个唯一任务定义、隔离准备器、评分器与 30-task 发布门汇总；6 个工程已在真实 REAPER 打开验证，两个混音工程已跑通渲染→确定性测量。G001 已完成两次现场 Agent 试跑：普通模式拒绝 `batch` 后未写入；修正信任策略后 12 次工具调用、2 个安全 `batch` 均零权限请求，但模型的对象式批参数曾造成额外 `Track 1`，该轮按失败和 false-success 记录。随后 reaper-mcp 已补对象参数规范化并通过离线自测；现场复跑前需在 REAPER 重新运行桥接脚本。真实运行驱动仍未固化，不能把 10 个定义记作 10 个已跑成绩；
...
1. 在 REAPER 重新运行桥接脚本后，先复跑 G001，验证 reaper-mcp 对象式批参数规范化后的零确认结果；
2. 固化 Phase 3.5 的真实 Agent 运行驱动，自动收集前后快照、工具事件、信任模式/明确许可和耗时证据；
3. 用现有 6 个固定工程跑出首批 10 个真实、可重复任务基线，再扩展到至少 30 个唯一任务；
4. 用 Gold Task 的渲染、测量和 A/B 结果关闭 Phase 3.4 最后一项，并安排真实制作人复核关键 Skill；
5. Eval 门通过后才开始 Phase 4；Phase 5 重新做当前主线的 GUI、MSI、升级与干净机验证。
